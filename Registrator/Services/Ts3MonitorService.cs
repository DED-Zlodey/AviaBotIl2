        ...
        try
        {
            ...
            foreach (var kvp in client.Book.Clients)
            {
                ...
                var existingLog = await db.UserAudioStateLogs
                    .FirstOrDefaultAsync(l => l.UserId == user.Id && l.ChangedAt > DateTimeOffset.UtcNow.AddMinutes(-1));
    ...
    private async void OnClientMoved(object? sender, ClientMoved e)
    {
        try
        {
            ...
            if (user != null)
            {
                ...
                {
                    var fromChannel = await db.TeamSpeakChannels.FirstOrDefaultAsync(c => c.TsChannelId == fromChid.Value);
