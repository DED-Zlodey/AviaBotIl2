...
public class Ts3RegistratorService : IHostedService, IDisposable
{
	...
	/// <summary>
	/// Провайдер доступа к клиенту TeamSpeak 3 для внешних сервисов (мониторинг и т.д.).
	/// </summary>
	...
	public Ts3RegistratorService(IOptions<RegistratorSettings> settings,
		IDbContextFactory<AppDbContext> dbContextFactory, ITs3ClientAccessor clientAccessor)
	{
	...
	private async Task RunBotAsync(CancellationToken ct)
	{
		try
		{
			...
			_tsFullClient.OnEachTextMessage += async (_, msg) =>
			{
				...
				{
					Log.Error(ex, "Error processing text message from {InvokerName} ({InvokerUid})", msg.InvokerName,
						msg.InvokerUid);
				}
			};
			...
			_clientAccessor.SetClient(_tsFullClient);
